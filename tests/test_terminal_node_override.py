...
class TestTerminalNodeOverride(unittest.TestCase):
    ...
    def test_terminal_node_ignores_child_yaml_files(self):
        ...
        convo2_file = inner_dialogue_folder / "convo2.txt"
        ...
        # Verify the explanation shows it's granted from the terminal node
        explanation = convo2_obj_refreshed.explain_permissions('andrew@openmined.org')
        self.assertIn("READ: ✓ GRANTED", explanation)
        self.assertIn(str(chat_folder), explanation)
    ...
    def test_terminal_node_with_specific_pattern(self):
        ...
        # Set up permissions: docs folder grants read to *.txt files
        docs_obj = syft_perm.open(docs_folder)
        docs_obj._grant_access("user1@example.com", "read", pattern="*.txt", force=True)
        docs_obj.set_terminal(True)
        
        # Try to revoke access in the private subfolder
        private_obj = syft_perm.open(private_folder)
        private_obj.revoke_read_access("user1@example.com", pattern="*.txt")
